...
describe("isArraywithEls", () => {
  it("returns false when there is no arg", () => {
    assert.isFalse(isArrayWithEls(), `${fail} false when there is no arg`);
  });
  it("return false when arg is not an array", () => {
    assert.isFalse(isArrayWithEls(""), `${fail} true when arg is ''`);
    assert.isFalse(isArrayWithEls({}), `${fail} true when arg is {}`);
    assert.isFalse(isArrayWithEls(true), `${fail} true when arg is true`);
    assert.isFalse(isArrayWithEls(0), `${fail} true when arg is 0`);
  });
